...
function createDesktopCallbackCapture(expectedState: string, timeoutMs = 5 * 60 * 1000): Promise<{
  callbackUrl: string;
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((request, response) => {
      const sendHtml = (statusCode: number, title: string, message: string) => {
        ...
        response.end(`<!doctype html><html><body style="font-family:Segoe UI,Arial,sans-serif;padding:24px"><h2>${title}</h2><p>${message}</p></body></html>`);
      };
      ...
      if (url.pathname !== "/callback") {
        sendHtml(404, "Not found", "Use the desktop callback URL generated by OpenClaw Desktop.");
        return;
      }
      ...
      if (errorText) {
        sendHtml(400, "Sign-in failed", `Auth service returned an error: ${errorText}`);
        return;
      }

      if (!state || state !== expectedState) {
        sendHtml(400, "Invalid callback", "State mismatch. Restart sign-in from the desktop app.");
        return;
      }

      if (!token) {
        sendHtml(400, "Invalid callback", "Missing token. Restart sign-in from the desktop app.");
        return;
      }

      sendHtml(200, "Connected", "OpenClaw Desktop is now linked. You can close this tab.");
      settle(null, token);
    });
...
async function runAuthHandoff(): Promise<AuthSessionStatus> {
  ...
  activeAuthHandoff = (async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const detail = typeof payload.error === "string" ? payload.error : `Exchange failed (${response.status}).`;
        return {
          baseUrl,
          reachable: true,
          authenticated: false,
